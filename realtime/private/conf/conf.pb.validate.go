...
// Validate checks the field values on Realtime with the rules defined in the
// proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *Realtime) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on Realtime with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in RealtimeMultiError, or nil
// if none found.
func (m *Realtime) ValidateAll() error {
	return m.validate(true)
}

func (m *Realtime) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if len(errors) > 0 {
		return RealtimeMultiError(errors)
	}

	return nil
}

// RealtimeMultiError is an error wrapping multiple validation errors returned
// by Realtime.ValidateAll() if the designated constraints aren't met.
type RealtimeMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m RealtimeMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m RealtimeMultiError) AllErrors() []error { return m }

// RealtimeValidationError is the validation error returned by
// Realtime.Validate if the designated constraints aren't met.
type RealtimeValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e RealtimeValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e RealtimeValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e RealtimeValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e RealtimeValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e RealtimeValidationError) ErrorName() string { return "RealtimeValidationError" }

// Error satisfies the builtin error interface
func (e RealtimeValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = RealtimeValidationError{}
